...
const Header = () => {
   ...
   const onClick = (name: SectionName) => {
   ...
   return (
      <header className=' relative z-[999]'>
         ...
         <nav className='fixed left-1/2 top-[0.15rem] flex h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0'>
            <ul className='sm:gap-p flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap'>
               {links.map((link) => (
                  <motion.li
                     key={link.hash}
